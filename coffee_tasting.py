...
# List of countries for drop-down multi-select
countries = ["Ethiopia", "Colombia", "Brazil", "Kenya", "Costa Rica", "Guatemala", "Sumatra", "Yemen", "Honduras", "Panama", "Peru", "Jamaica"]
...
with st.form(key="tasting_form", clear_on_submit=True):
    ...
    roasted_at = st.text_input("Where the Coffee Was Roasted")
    origins = st.multiselect("Select Bean Origin Countries", countries)  # New multi-select field for bean origins
...
if submit_button:
    new_entry = {
        ...
        "Bean Origin Countries": ", ".join(origins),  # Store multiple selected countries
        "Acidity": acidity,
        "Sweetness": sweetness,
        "Body": body,
...
if not data.empty:
    ...
    with st.form(key="edit_entry_form"):
        ...
        edited_roasted_at = st.text_input("Edit Where Roasted", value=data.iloc[selected_index].get("Roasted At", ""))
        ...
        if update_button:
            ...
            data.at[selected_index, "Bean Origin Countries"] = ", ".join(edited_origins)  # Save multiple countries as a comma-separated string
